...
export default function DashboardNutricionista({ navigation }) {
  const [nome, setNome] = useState('');
  const { logout } = useAuth();

  // === CARREGAR PERFIL DO NUTRICIONISTA ===
  useEffect(() => {
    async function carregarPerfil() {
      try {
        const perfil = await api.get('/usuarios/perfil');
        setNome(perfil.data.nome || 'Nutricionista');
      } catch (err) {
        console.error('Erro ao carregar perfil do nutricionista:', err);
        Alert.alert('Erro', 'Falha ao carregar dados do nutricionista.');
      }
    }
    carregarPerfil();
  }, []);
  ...
  // === CARDS DISPONÍVEIS PARA O NUTRICIONISTA ===
  const cards = [
    {
      title: 'Agendamentos',
      icon: 'calendar-alt',
